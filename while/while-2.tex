...
\objectifs{Our study of numbers is further developed with the loop \og{}while\fg{}. For this chapter you will need the function \ci{is_prime()} you wrote in the part
\og{}Arithmetic -- While loop -- I\fg{}.}
...
\objectifs{Goal: study two Goldbach conjectures. A conjecture is a statement that you think is true but you can't prove it.}
\index{Goldbach}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Program a function \ci{number_solutions_goldbach(n)} which for a given even integer $n$, finds how many decompositions $n=p+q$ there are with $p$ and $q$ two prime numbers and $p\le q$.
...
where $p$ is a prime number and $k$ is an integer (possibly zero).}
  ...
  \begin{enumerate}
    \item Program a function \ci{is_decomposition_goldbach(n)} that returns \og{}True\fg{} when there is a decomposition of the form $n=p+2k^2$.
...
\begin{enumerate}
  ...
  Program a function \ci{number_of_divisors(n)} that returns the number of integers dividing $n$.
  ...
  \begin{itemize}
   ...
   Program a function \ci{four_and_eight_divisors(Nmin,Nmax)} that returns two numbers: (1) the number of integers $n$ with $N_{\text{min}} \le n < N_{\text{max}}$ that admit exactly $4$ divisors and (2) the number of integers $n$ with $N_{\text{min}} \le n < N_{\text{max}}$ that admit exactly $8$ divisors.
...
\begin{enumerate}
 ...
 \emph{You might think it's still the case, but it's not true. The integer $U_{136}$ is a prime number! Unfortunately it is too big to be verified with our algorithms. In the following we will define what is a \emph{almost prime number} to be able to push the calculations further.}
  ...
  \item Program a function \ci{is_almost_prime(n,r)} that returns \og{}True\fg{} if the integer $n$ does not admit any divisor $d$ such as $1< d \le r$ (we assume $r<n$). 
  ...
  \emph{Hint.} Adapt your function \ci{is_prime(n)}!
...
\begin{enumerate}
  ...
  \emph{Hints.}
  \begin{itemize}
    \item For this question only, you can use the module \ci{math} of \Python. 
    \item In this module \ci{sqrt()} returns the real square root.
    \item The function \ci{floor()} of the same module returns the integer part of a number.
  \end{itemize}
...
\textbf{Bonus.} Compare the execution speeds of the three methods using \ci{timeit()}. See the chapter \og{}Functions\fg{}.
...
\ci{break}\index{break@\ci{break}}\index{loop!quit}.

Here are some examples that use this command \ci{break}. As it is rarely an elegant way to write your program, alternatives are also presented.
