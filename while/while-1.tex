...
\objectifs{The activities in this sheet focus on arithmetic: Euclidean division, prime numbers \ldots{} This is an opportunity to use the loop \og{}while\fg{} intensively.}
...
\objectifs{Goal: use the remainder to find out if an integer divides another.}


\begin{enumerate}
  \item Program a function \ci{quotient_remainder(a,b)} that does the following tasks for two integers $a\ge0$ and $b>0$ :
  \begin{itemize}
...
Here is for example what the call should display \ci{quotient_remainder(100,7)}:
\begin{lstlisting}  
...
\emph{Note.} You have to check without cheating that we have $0 \le r<b$ and $a=bq+r$, but of course it must always be true!
  ...
  \item Program a function \ci{is_even(n)} that tests if the integer $n$ is even or not. The function returns \ci{True} or \ci{False}.
  
  \emph{Hints.}
  \begin{itemize}
    \item First possibility: calculate \ci{n \% 2} and discuss as appropriate.
    \item Second possibility: calculate \ci{n \% 10} (which returns the digit of units) and discuss.
  ...
  \item Program a function \ci{is_divisible(a,b)} that tests if $b$ divides $a$. The function returns \ci{True} or \ci{False}.
...
\begin{exemple}
\begin{minipage}{0.55\textwidth}
For this last loop we have already prepared a function \ci{is_even(n)} which returns \ci{True} if the integer $n$ is even and \ci{False} otherwise. The loop does this: as long as the integer $n$ is even, $n$ becomes $n/2$. This amounts to removing all factors $2$ from the integer $n$. As here $n = 56 = 2\times 2 \times 2 \times 7$, this program displays $7$.
\end{minipage}\qquad\qquad
\begin{minipage}{0.4\textwidth}
\begin{lstlisting}
n = 56
...
\begin{enumerate}
  ...
  Program a function \ci{smallest_divisor(n)} that returns, the smallest divisor $d\ge2$ of the integer $n\ge2$.
  ...
  Slightly modify your function \ci{smallest_divisor(n)} to write a first function \ci{is_prime_1(n)} which returns \og{}True\fg{} if $n$ is a prime number and \og{}False\fg{} otherwise.
  ...
  \item \textbf{Prime numbers (2).}
   ...
   Enhance your previous function to a \ci{is_prime_2(n)} that does not test all divisors $d$ from $2$ to $n$, but only up to $\sqrt{n}$.
  ...
  Improve your function into a function \ci{is_prime_3(n)} using the following idea.  We test if $d=2$ divides $n$, but from $d=3$, we just test the odd divisors (we test $d$, then $d+2$\ldots).
 ...
 \begin{itemize}
  ...
  Compare the calculation times of your different functions \ci{is_prime()} by repeating for example a million times the call \ci{is_prime(97)}. See the course below for more information on how to do this.
\end{enumerate}   
...
\objectifs{Goal: program more loops \og{}while\fg{} and study different kinds of prime numbers using your function \ci{is_prime()}.}
