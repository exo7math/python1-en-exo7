...
\begin{itemize}
  ...
  \item The text is displayed by the command \ci{canvas.create_text()}. It is necessary to specify the coordinates $(x,y)$ of the point from which you want to display the text. 
...
\begin{enumerate}
  \item 
  \begin{enumerate}
    \item Define a window \ci{tkinter} of size $800 \times 600$ for example.
  ...
  \item Write a function \ci{text_box(word,font)} that draws a rectangle around a text. To do this, use the \ci{canvas.bbox(myobject)} method which returns the $x_1,y_1,x_2,y_2$ coordinates of the desired rectangle. (Here \ci{myobject = canvas.create_text(...)}).
  
  \item Write a function \ci{length_word(word,font)} that calculates the length of a word in pixels (this is the width of the rectangle of the previous question).
  
  \item Write a function \ci{font_choice(mode,in_bold,in_italics)} that returns the name of an adapted font (among those defined in the first question) according to a mode (among \ci{"title"}, \ci{"subtitle"}, \ci{"text"}) and according to booleans \ci{in_bold}, \ci{in_italics}.
...
\end{center}    
  ...
  \emph{Hints.}
    ...
    \item For lists, you can get the bullet character \og{}\textbullet\fg{} by the unicode character \ci{u'\\u2022'}\index{unicode}. You can also indent each item of the list for more readability.
    
    \item Use the \ci{font_choice()} function of the first activity.
    
    \item In the image above, each line is produced by a call to the function. For example \ci{print_line_v2("## And there a subtitle",100)}.
   \end{itemize}
...
\end{center}   
  ...
  \begin{itemize}
    ...
    \item Define a boolean variable \ci{in_bold} that is false at the beginning; each time you encounter the tag \ci{**} then reverse the value of \ci{in_bold} (\og{}True\fg{} becomes \og{}False\fg{}, \og{}False\fg{} becomes \og{}True\fg{}, you can use the operator \ci{not}).
    
    \item Still use the \ci{font_choice()} function of the first activity.
    
    \item In the image above, each line is produced by a call to the function. For example \ci{print_line_v3("+ Apples and ** pears ** and * ananas *",100)}
   \end{itemize} 
...
\end{center}   
  ...
  \begin{itemize}
   ...
   \item Ends with a function \ci{print_file(filename)} that displays the paragraphs of a text file with our simplified \emph{Markdown} syntax.
\end{enumerate} 
\end{activite}
...
\begin{enumerate}
  \item Write a function \ci{hyphenation_simple()} that calculates the indices to make the hyphenations corresponding to the figure below, i.e. an alignment on the left (without spaces) and without exceeding the total length of the line (here of length $100$).
  
\begin{center}
\includegraphics[scale=0.5]{screen-hyphen-2}
\end{center} 

  \begin{fonction}[\ci{hypenation_simple()}]
  Use: \ci{hypenation_simple(list_lengths)} \\
  Input: a sequence of lengths (a list of integers)\\
  Output: the list of indices where to make a cut
  \medskip
    
  Example: \ci{hypenation_simple(list_lengths)} where \ci{list_lengths} is the example given above, returns the list \ci{[0, 9, 17, 27, 39, 49, 50]}. That is to say that:
  \begin{itemize}
  ...
  \item Modify your work into a function \ci{hyphenation_spaces()} that adds a space (with \ci{space_length = 1}) between two words of the same line (but not at the beginning of the line, nor at the end of the line). This corresponds to the following drawing:
...
Write a function \ci{compute_space_lengths()} that returns the length that the spaces of each line must have in order for the text to be justified.
...
To find the right formula, simply take the results of the \ci{hyphenation_spaces()} function and then, for each line, count the total length of words it contains, as well as the value it lacks to arrive at a total of $100$.
\end{enumerate} 
\end{activite}
...
\objectifs{You now have everything you need to visualize text written with the syntax \emph{Markdown} and justify it (see the displayed text of Darwin in the lesson). It's still a lot of work! You can also improve the support of \emph{Markdown} syntax: code, numbered lists, sublists, bold and italic words at the same time\ldots}
