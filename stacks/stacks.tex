...
\begin{enumerate}
  \item Write a function \ci{push_to_stack()} that adds an element to the top of the stack.
...
\ci{push_to_stack(6)}, the stack is now \ci{[5,1,3,8,6]}.     
  ...
  \item Write a function \ci{pop_from_stack()}, without parameters, that removes the element at the top of the stack and returns its value.
  ...
  \item Write a function \ci{is_stack_empty()}, without parameter, that tests if the stack is empty or not. 
...
\objectifs{Goal: handle the stacks using only the three functions \ci{push_to_stack()}, \ci{pop_from_stack()} and \ci{is_stack_empty()}.}
...
\end{enumerate}  
  ...
  \item Start from a stack. Write a function \ci{is_in_stack(item)} that tests if the stack contains a given element.
...
\begin{enumerate}
  ...
  \item The function \ci{isdigit()}\index{isdigit@\ci{isdigit}} is a \Python{}    method that tests if a string contains only numbers. This allows to test if a string corresponds to a positive integer.
 Here are some examples: \ci{"1776".isdigit()} returns \ci{True}; \ci{"Hello".isdigit()} returns \ci{False}.
 
 Remember that you can convert a string into an integer by the command \ci{int(string)}. The following small program tests if a string can be converted into a positive integer:
...
\begin{itemize}
  ...
  \item The list of wagons is obtained by asking \ci{train.split()}.
  
  \item We test if a wagon is blue and check if it is marked with a number, by the function \ci{wagon.isdigit()}.
...
\objectifs{Writing in Polish notation (of its real name, reverse Polish notation) is another way to write an algebraic expression. Its advantage is that this notation does not use brackets and is easier to handle for a computer. Its disadvantage is that we're not used to it.}
...
\objectifs{Goal: determine if the parentheses of an expression are placed appropriately.}
...
Here is the algorithm to program a function \ci{are_brackets_balanced()}.
...
\objectifs{Goal: transform a classic algebraic expression with parentheses into a Polish notation expression. The algorithm is a much improved version of the previous activity. We will not give any justification.}
...
\begin{itemize}
  ...
  \item With your calculator calculates the result: \ci{polish_calculator("17 2 3 + * 4 8 5 * + +")} returns \ci{129}. We get the same result!
  \end{itemize}
