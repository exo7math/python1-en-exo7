...
\objectifs{The \emph{game of life} is a simple model of the evolution of a population of cells that split and die over time. The \og{}game\fg{} consists in finding initial configurations that give interesting evolution: some groups of cells disappear, others stabilize, some move\ldots}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item By instructions of the type \ci{array[i][j] = 1}, fill in the table to define the configuration of the blinker, the toad \ldots
  \end{itemize}
...
\end{center}
  ...
  \emph{Hint.} By default the command \ci{print()} passes to the next line on each call (it adds the character \ci{"\\n"} which is the end of line character). It can be specified not to do so by the option
  \ci{print("My text",end="")}.
...
\begin{enumerate}
  ...
  \item Build a function \ci{draw_array(array)} that graphically displays the cells from a table. 
...
\objectifs{Goal: calculate the evolution of a configuration to the next day.}

\begin{enumerate}
  \item Program a function \ci{number_neighbors(i,j,array)} that calculates the number of living neighboring cells to the cell $(i,j)$.
  ...
  \item Program a function \ci{evolution(array)} that receives a table at the input and returns a new array corresponding to the situation on the next day, according to the rules of the game of life described at the beginning.
...
\objectifs{Goal: end the graphic program so that the user can define configurations and make them evolve with a simple click.}
...
\end{center}  
  ...
  \begin{itemize}
    ...
    \item \ci{xy_to_ij(x,y)} that converts graphic coordinates $(x,y)$ into integers coordinates $(i,j)$ (use variable \ci{scale} and the integer division).
...
\begin{itemize}
  ...
  \item Add an intermediate step before evolving: color in green a cell that will be born and in black a cell that will die. For this purpose the elements of \ci{array} can take values other than $0$ and $1$.
\end{itemize}
