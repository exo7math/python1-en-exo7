...
\end{center}
  ...
  \emph{Hint.} By default the \ci{print()} start a new line on each call (i effectively adds \ci{"\\n"} which is the end of line character). It can be specified not to do sthis by the option \ci{print("My text",end="")}.
...
\begin{enumerate}
  ...
  \item Program a \ci{evolution(array)} function that receives a table as an input and returns a new array corresponding to the situation on the next day, according to the rules of the game of life described at the beginning.
