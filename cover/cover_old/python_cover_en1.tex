...
% xelatex -output-driver="xdvipdfmx" python_cover_en1.tex
%
...
\begin{document}%
  \begin{tikzpicture}
    ...
    \begin{scope}[scale=0.7,transform canvas ={yshift=21mm}]
      ...
      % ---------------------------------------------------
      \path[every node/.style={scale=4, font=\titlefont, outer sep=0, inner sep=0}]
        (Front.center) +(21mm,21mm) node[titlepython]{PYTHON}
        (Front.center) +(-69mm,-4mm) node {\textcolor{titleau}{IN}}
        (Front.center) +(-21mm,-21mm) node {\textcolor{titlelycee}{HIGHSCHOOL}   
        };
      
%      \path (Cover.east) +(55mm,-125mm) node[left,scale=1.8,text width=.40\PageW,align=left,fill=white,text=subtitlealgo,inner sep=.3em,font=\authorfont,rounded corners=10pt]{\bf ~~Algorithms and mathematics~~\vphantom{\Large t}};

\path (Cover.east) +(55mm,-145mm) node[left,scale=1.8,text width=.40\PageW,align=left,fill=white,text=subtitlealgo,inner sep=.3em,font=\authorfont,rounded corners=10pt]{\bf ~~Algorithms and Mathematics~~\vphantom{\Large t}};
...
%        {\textcolor{spinepython}{PYTHON}\textcolor{spineau}{IN}\textcolor{spinelycee}{HIGHSCHOOL}\textcolor{spinealgo}{~~~\textemdash~~tome 1}\vphantom{\large y}};
%    \end{scope}
    \begin{scope}[transform canvas ={yshift=21mm}]
      \node[rotate=90,scale=1.8] (tranche) at (Spine.center)
        {\textcolor{spinepython}{PYTHON}\textcolor{spineau}{IN}\textcolor{spinelycee}{HIGHSCHOOL}\vphantom{\large y}};
    \end{scope}    
...
\emph{Python} is the ideal language to learn programming.
 It is a powerful language that will immerse you in the world of algorithms. 

\bigskip

This book guides you step by step through original mathematical and computer activities adapted to high school. It is complemented by online resources: all the \emph{Python} codes and colourful chapters. 
...
\item Hello world!
\item Turtle (Scratch with Python)
\item If ... then ...
\item Functions
\item Arithmetic -- While loop -- I
\item Strings -- Analysis of a text
\item Lists I
\item Statistics -- Data visualization
\item Files
\item Arithmetic -- While loop -- II
\item Binary I
\item Lists II
\item Binary II
\end{itemize}
...
\item Probabilities -- Parrondo’s paradox
\item Find and replace
\item Polish calculator -- Stacks
\item Text viewer -- Markdown
\item L-systems
\item Dynamic images
\item Game of life
\item Ramsey graphs and combinatorics
\item Bitcoin
\item Random blocks
\end{itemize}
\end{minipage}
