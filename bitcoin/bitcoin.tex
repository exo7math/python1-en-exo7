...
\begin{enumerate}
  ...
  Write a function \ci{verification(x,y)} that returns \og{}True\fg{} if $x$ is the solution to the problem $x^2 = y \pmod{p}$ and \og{}False\fg{} otherwise.
...
\emph{Conclusion: we found a problem that was difficult to solve, but for which it is easy to check that a given solution is suitable. For higher values of $p$, the search for a solution $x$ can be much too long and not succeed. We'll see how we can adjust the difficulty of the problem.}
	...
	Program a function \ci{approximate_square_root(y,margin)} that finds an approximate solution to our problem $x^2 = y \pmod{p}$.
...
The \ci{time} module allows you to measure the execution time but also to know the date and time (see also the \ci{timeit} module explained in the \og{}Arithmetic -- While loop -- I\fg{}).
...
\begin{itemize}
  ...
  \item The function \ci{time()} returns a number that we are not interested in. What interests us is the difference between two values of this function. 
...
\begin{enumerate}
  \item Program a function \ci{addition(mylist1,mylist2)} that adds term to term and modulo $100$, the elements of two lists of the same length. 
  ...
  Program a function \ci{is_smaller(mylist,max_list)} that returns \og{}True\fg{} when \ci{mylist} is smaller than \ci{max_list}.
  ...
  Write a function \ci{sentence_to_list(sentence)} that converts a string into a list of integers between $0$ and $99$ and if necessary adds leading zeros so that the list has the correct size. 
...
\tikzinput{fig-bitcoin-1}
  ...
  Program such a function \ci{one_round(block)} which returns the transformation of the block after these operations. Verifies that the block $[1, 1, 2, 3, 4, 5]$ is transformed into $[8, 8, 23, 27, 86, 77]$.
...
\tikzinput{fig-bitcoin-2}
  ...
  \begin{itemize}
    ...
    \item This mix is added to the second block $[1,1,1,1,1,1]$ (see function \ci{addition()} in activity 2).
...
For example: let \ci{mylist = [0,1,2,3,4,5]} and \ci{max_list = [0,0,7]}. Which block \ci{proof} can I concatenate to \ci{mylist} to solve my problem?
\begin{itemize}
...
\begin{enumerate}
  ...
  Program a function \ci{verification_proof_of_work(mylist,proof)} that returns true if the proposed solution \ci{proof} is suitable for \ci{mylist}. Use the \ci{is_smaller()} function of activity 2.
  
  \item \textbf{Search for a solution (difficult).}
  
  Program a function \ci{proof_of_work(mylist)} that looks for a block \ci{proof} solution to our problem for the given list. 
  
  
  \emph{Hints.}
  
  \begin{itemize}
    \item The easiest method is to take a block \ci{proof} of random numbers and start again until a solution is found.
    ...
    \item You adjust the difficulty of the problem by changing the objective: easy with \ci{Max = [0,0,50]}, medium with \ci{max_list = [0,0,5]}, difficult with \ci{max_list = [0,0,0]}, too difficult with \ci{max_list = [0,0,0,0]}.
   ...
   Compare the calculation time of a simple check against the time of searching for a solution. Choose the objective \ci{max_list} so that the search for a proof of work requires about 30 to 60 seconds of calculations.
...
\objectifs{Goal: create an account book (called \emph{blockchain} for the bitcoin) that records all transactions, this register is public and certified. It is practically impossible to falsify a transaction that has already been registered.}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Initialize a global variable \ci{blockchain} which is a list and contains at the beginning a zero proof: \ci{blockchain = [ [0,0,0,0,0,0] ]} .
    ...
    Program a function \ci{add_transaction(transaction)} that adds the string \ci{transaction} to the list \ci{blockchain}. 
...
Here's how to do it:
  \begin{itemize}
    \item We take the last transaction \ci{transaction}, we transform it into a list of integers by the function \ci{sentence_to_list()} of activity 2.
    \item We also need \ci{prev_proof}, the previous proof of work just before this transaction.
    \item We form the list \ci{mylist} composed first of elements of \ci{prev_proof}, then of elements of the list of integers obtained by converting the string \ci{transaction}.
  ...
  \mycenterline{\ci{[3,1,4,1,5,9], "Abel +35", [32,17,37,73,52,90]}}  
     ...
     It should be remembered that proof of work is not unique and that it also depends on the objective \ci{max_list}.
   ...
   Only one person at a time adds a proof of work. However, everyone has the opportunity to verify that the proposed proof is correct (and should do it). 
	Write a function \ci{verification_blockchain()}, without parameters, that checks that the last proof added to the \ci{blockchain} is valid.
