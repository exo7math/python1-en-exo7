...
On the left is a program that writes two lines in a file called \ci{my_file.txt}; on the right is the resulting file that is displayed in a text editor.
\begin{center}
...
\begin{itemize}
  ...
  \item We do not work with the file name, but with the value returned by the function \ci{open}. Here we have named this object file \ci{fi}. It is with this variable \ci{fi} that we work now.
  ...
  \item The data to be written are strings, so to write a number, you must first transform it with \ci{str(number)}.\index{str@\ci{str}}
\end{itemize}
...
\textbf{Explanations.}
\begin{itemize}
  \item The command \ci{open} is this time called with the argument \ci{"r"} (for read), it opens the file in reading.
  
  \item We work again with an object file named here \ci{fi}.
...
\begin{enumerate}
  \item Generates at random a grades file, named \ci{grades.txt}, which is composed of lines with the structure: 
  \mycenterline{\ci{Firstname Name grade1 grade2 grade3}} 
...
%\end{center} 
  ...
  \mycenterline{\ci{Firstname Name average}} 
...
%\end{center}  
  ...
  \emph{Hints.}
  \begin{itemize}
    	\item For each line read from the file \ci{grades.txt}, you get the data as a list by the command \ci{line.split()}.
    	
    	\item \textbf{Please note!} The data read is a string. You can convert a string \ci{"12.5"} to the number \ci{12.5} by the instruction \ci{float(string)}.
...
\objectifs{Goal: write a data file, with \emph{csv} format, then read it for a graphic display.}

\begin{enumerate}
  \item Generates a \ci{sales.csv} file of sales figures (randomly drawn) from a sports brand.
...
\end{center} 
  ...
  \item Reads the file \ci{sales.csv} to display the sales curves.
  \begin{center}
...
Here is an example: the file \ci{image_bw.pbm} on the left (read as a text file) and on the right its visualization (using an image reader, here very enlarged).
\begin{center}
\begin{minipage}{0.3\textwidth}
\begin{lstlisting}
P1
...
Here is an example: the file \ci{image_gray.pbm} on the left and on the right its visualization.
\begin{center}
\begin{minipage}{0.3\textwidth}
\begin{lstlisting}
P2
...
\begin{itemize}
	...
	\item For the formulas, it will be necessary to use the angles in radians. The formula for converting degrees to radians is: \index{angle}\index{degrees}\index{radians}
