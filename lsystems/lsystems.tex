...
For example when \ci{word = "ArBlA"}, \ci{replace_2(word,"A","ABl","B","Br")} returns the word \ci{ABlrBrlABl}: each letter \mot{A} has been replaced by the pattern \mot{ABl} and at the same time each letter \mot{B} has been replaced by the \mot{Br}.
