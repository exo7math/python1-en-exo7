...
\objectifs{The L-systems offer a very simple way to code complex phenomena. From an initial word and a number of replacement operations, we arrive at complicated words. When you \og{}draw\fg{} these words, you get beautiful fractal figures. The \og{}L\fg{} comes from the botanist A.~Lindenmayer who invented the L-systems to model plants.}
...
A \defi{L-system} is the data of an initial word and replacement rules.
Here is an example with the starting word and only one rule: 
\mycenterline{\mot{BlArB} \qquad \mot{A} $\rightarrow$ \mot{ABA}}


The \defi{$k$-iteration} of the L-system is obtained by applying $k$ times the substitution to the starting word.
With our example:
\begin{itemize}
...
\begin{itemize}
  \item The command \ci{draw(100)} draws my line, and as I only specified the length, the arguments \ci{width} and \ci{color} take the default values ($5$ and blue).
  
   \item The command \ci{draw(100, width=10)} draws my line with a new thickness (the color is the default one).
   
    \item The command \ci{draw(100, color="red")} draws my line with a new color (the thickness is the default).  
     ...
     \begin{itemize}
       \item \ci{draw(100, 10, "red")}: do not specify the names of the options if you pay attention to the order.
       \item \ci{draw(color="red", width=10, length=100)}: you can name any variable, the named variables can pass as arguments in any order.
...
You are given a word (for example \mot{AlArAArArA}) in which each letter (read from left to right) corresponds to an instruction for the \Python{} turtle.

\begin{itemize}
  \item \mot{A} or \mot{B}: advance of a fixed quantity (by tracing),
...
Program a function \ci{draw_lsystem(word,angle=90,scale=1)}
which displays the drawing corresponding to the letters of \ci{word}. By default the angle is $90$ degrees, and each time you advance, it is of $100 \times$\ci{scale}.
...
\begin{enumerate}
  \item Program a function \ci{replace_1(word,letter,pattern)} that replaces a letter with a pattern in a word. 
...
For example with \ci{word = "ArAAl"}, \ci{replace_1(word,"A","Al")} returns the word \ci{AlrAlAll}: each letter \mot{A} has been replaced by the pattern \mot{Al}.
  ...
  \item Program a function \ci{iterate_lsystem_1(start,rule,k)}
  which calculates the $k$-iteration of the L-system associated with the initial word \ci{start} according to the rule \ci{rule} which contains the couple formed by the letter and its replacement pattern.
  For example, with:
  \begin{itemize}
    \item \ci{start = "A"}
  ...
  For $k=1$, the word is \mot{AlArArAlA} and you can check the trace on the first image.
...
\objectifs{Goal: calculate more complicated L-systems by allowing two replacement rules instead of one.}
...
\begin{enumerate}
  \item Program a function \ci{replace_2(word,letter1,pattern1,letter2,pattern2)} that replaces a first letter with a pattern and a second letter with another. 

For example with \ci{word = "ArBlA"}, \ci{replace_2(word,"A","ABl","B","Br")} returns the word \ci{ABlrBrlABl}: each letter \mot{A} has been replaced by the pattern \mot{ABl} and at the same time each letter \mot{B} has been replaced by the \mot{Br}.

\emph{Warning!} Do not get \ci{ABrlrBrlABrl}. If this is the case, it is because you used the \ci{replace_1()} function to first replace the \ci{A}, then a second time for the \ci{B} (but after the first replacement new \ci{B} appeared). A new function must be reprogrammed to avoid this. 

 
  \item Program a function  \ci{iterate_lsystems_2(start,rule1,rule2,k)}
...
The letters \ci{X} and \ci{Y} do not correspond to any action.

	\item A variant of the Sierpinski triangle, with \ci{angle = 60} :	
\mycenterline{\ci{	
start="A"    
rule1=("A","BrArB")    
rule2=("B","AlBlA")
...
\item The Gosper curve, with \ci{angle = 60}:
...
\begin{cours}[Stacks]
A \defi{stack} is a temporary storage area. Details are in the chapter \og{}Polish calculator -- Stacks\fg{}. Here are just a few reminders.
...
\begin{itemize}
  ...
  \item \textbf{Push.} We add the items at the end of the list: \ci{stack.append(element)} or \ci{stack = stack + [element]}.
  \item \textbf{Pop.} An item is removed by the command \ci{pop()}\index{pop@\ci{pop}}:  
...
\objectifs{Goal: improve our tracings by allowing us to move forward without tracing and also by using a kind of flashback, to trace plants.}
...
\begin{enumerate}
  ...
  Increase the possibilities by allowing the turtle to move forward without drawing a line, when the instruction is the letter \mot{a} (in lowercase). (It is sufficient to modify the
function \ci{trace_lsystems()}.)
...
\end{center}  
  ...
  We now allow square brackets in our words. For example \mot{AlA[lAAA]A[rAA]A}. When you encounter a opening bracket \og{}\mot{[}\fg{}, you memorize the position of the turtle, then the commands in brackets are executed as usual, when you find the closing bracket \og{}\ci{]}\fg{} you go back to the position stored before.
  
  
Let us understand the example of the route of the 
\mot{
{\color{blue}AlA}
{\color{red}[lAAA]}
...
\begin{itemize}
  ...
  \item \mot{{\color{red}[lAAA]}}: we retain the current position (the point $P$) and also the direction; we turn, we advance three times (we trace the red segment); at the end we return the turtle to the position $P$ (without tracing and with the same direction as before).
  \item \mot{{\color{green!70!black}A}}: from $P$ we advance (green segment).
  \item \mot{{\color{purple}[rAA]}}: we retain the position $Q$ and the direction, we turn and we trace the purple segment. We come back in $Q$ with the old state.
...
\end{itemize}
  ...
  \begin{itemize}
	...
	\item We read one by one the characters of the word. The actions are the same as before.
	\item If the character is the opening bracket \og{}\mot{[}\fg{} then add to the stack the current position and direction of the turtle $\big( (x,y), \theta\big)$ that you get by \ci{(position(), heading())}.
	
	\item If the character is the closing bracket \og{}\mot{]}\fg{} then pop 
	(i.e. read the top element of the stack and remove it). Put the position of the turtle and the angle with the read values, use \ci{goto()} and \ci{setheading()}.
