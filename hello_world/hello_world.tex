...
\objectifs{Get into programming! In this very first activity, you will learn to manipulate numbers, variables and code your first loops with \Python.}
...
\objectifs{Goal: make your first calculations with \Python.}
...
From now on, it is better if you work in a text editor dedicated to \Python. You must then explicitly ask to display the result\index{print@\ci{print}}:
\begin{lstlisting}
print(2+2)
print("Hello world!")
\end{lstlisting}
...
\end{center}  
  ...
  \item I define two variables by \ci{a = 9} and \ci{b = 11}. I would like to exchange the content of \ci{a} and \ci{b}. Which instructions agree so that at the end \ci{a} equals $11$ and \ci{b} equals $9$?
...
\end{center} 	 
  ...
  There are many other functions. For example, the function \ci{abs()} calculates the absolute value: \ci{abs(-3)} returns \ci{3}, \ci{abs(5)} returns \ci{5}.
...
The first line imports all the functions of the module named \ci{math}\index{import@\ci{import}}, the second line calculates $x = \sqrt{2}$ (in approximate value) and then displays $x$ and $x^2$.	

  \item \textbf{Sine and cosine.} 
  
  The module \ci{math} contains the trigonometric functions sine \index{sin@\ci{sin}} and cosine \index{cos@\ci{cos}} and even the constant \ci{pi}\index{pi@\ci{pi}}\index{$\pi$} which is an approximate value of $\pi$. Be careful, the angles are expressed in radians.
...
In the module \ci{math} there are also functions to round a decimal number: 
  \begin{itemize}
...
\objectifs{Goal: use functions from \Python{} and the module \ci{math}.}

\begin{enumerate}
  \item The function \Python{} for gcd is \ci{gcd(a,b)}\index{gcd@\ci{gcd}} (for greatest common divisor). Calculate the gcd of $a = 10\,403$ and $b = 10\,506$. Deduce the lcm\index{lcm} from $a$ and $b$. The function lcm does not exist, you must use the formula :
...
\begin{cours}[Loop \og{}for\fg{}]

\objectifs{The loop \og{}for\fg{} is the easiest way to repeat instructions.}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item For $\pi$ you can take the approximate value $3.14$ or the approximate value \ci{pi} of the module \ci{math}.
  \end{itemize}
...
\begin{activite}[Loop \og{}for\fg{} (continued)]
...
\begin{enumerate}
  ...
  \emph{Hints.} Begin by defining a variable \ci{myproduct} initialized to the value $1$. Use \ci{range(a,b,2)} to get every other integer.
