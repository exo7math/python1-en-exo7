...
\objectifs{The computers transform all data into numbers and manipulate only those numbers. These numbers are stored in the form of lists of $0$'s and $1$'s. It's the binary numeral system of numbers. To better understand this binary numeral system, you will first understand the decimal numeral system better.}
...
Write a function \ci{decimal_to_integer(list_decimal)} which from a list representing the decimal notation calculates the corresponding integer.
...
\index{power of 2}
  ...
  \item \textbf{Binary system: an example.}
    ...
    Integers admit a binary numeral system, i.e. a notation where the only numbers are $0$ or $1$. In binary, the digits are called \defi{bits}.\index{bits}
...
$$n = {\color{red}b_{p-1}} \times 2^{p-1} + {\color{red}b_{p-2}} \times 2^{p-2} + \cdots + {\color{red}b_i} \times 2^i +  \cdots + {\color{red}b_2} \times 2^2 + {\color{red}b_1} \times 2^1 + {\color{red}b_0} \times 2^0$$
    ...
    \item \textbf{\Python{} and the binary.} \Python{} accepts that we directly write the integers in binary numeral system, just use the prefix \og{}\ci{0b}\fg{}.
    Examples:
    \begin{itemize}
...
\objectifs{Goal: find the integer from a binary notation.}

\begin{enumerate}

  \item Calculate integers whose binary numeral system is given below. You can do it by hand or help yourself from \Python. For example $1.0.0.1.1$ is equal to $2^4+2^1+2^0 = 19$ as confirmed by the command \ci{0b10011} which returns $19$.
  ...
  \item Write a function \ci{binary_to_integer(list_binary)} which from a list representing binary system calculates the corresponding integer.
...
\begin{cours}[Base $10$ system (bis)]
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item calculate by \ci{n \% 10} the \defi{remainder}, we also call it \defi{$n$ modulo $10$}
    \item calculate \ci{n // 10} the \defi{quotient} of this division. 
  \end{itemize}
...
\tikzinput{fig-binary-1}
  ...
  \item Commands with \Python{} are simply \ci{n \% 10} and \ci{n // 10}.
...
\objectifs{Goal: decompose an integer into a list of its figures (based on $10$).}

Program the following algorithm into a function \ci{integer_to_decimal()}.
...
\Python{} calculates very well the binary numeral system of an integer using the function \ci{bin()}.
\index{bin@\ci{bin}}
...
\objectifs{Goal: find the binary numeral system of an integer.}

\begin{enumerate}
  \item Calculate by hand the binary numeral system of the following integers. Check your results using the \ci{bin()} \Python{} function.
  ...
  \item
  Program the following algorithm into a function \ci{integer_to_binary()}.
