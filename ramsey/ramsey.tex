...
\objectifs{Proposition. In a group of $6$ people, there is always $3$ friends (the three know each other) or $3$ foreigners (all three are strangers to each other).}
...
\begin{enumerate}
  \item Program a \ci{subsets(n)} function which returns the list of all possible subsets of $E_n =  \{0,1,2,\ldots,n-1\}$.
...
\emph{Hint.} To test your program, check that the returned list contains $2^n$ subsets.
  ...
  \item Deduce  from it a \ci{fix_subsets(n,k)} function that returns only subsets of $E_n$ having exactly $k$ elements.
...
\begin{enumerate}
  \item Program a \ci{have_3(array)} function that tests if a graph contains $3$ friends or $3$ foreigners. You must therefore call the functions \ci{have_3_fix_friends(array,i,j,k)} and \ci{have_3_fix_strangers(array,i,j,k)} of the first activity for all possible triples of vertices $(i,j,k)$.
 
 For the four examples of the first activity, only the fourth (with $6$ summits) passes the test.
  
  \item Program an \ci{all_graphs(n)} function that computes all possible graph arrays with $n$ vertices. There are $N = \frac{(n-1)n}{2}$ possible arrays. You can generate them by a method similar to the one for subsets:
  \begin{itemize}
...
\emph{Hints.}
To fill an array from a given binary notation of $p$ named \ci{binary_notation} (that is to say a list of $0$'s and $1$'s), you can use a double loop like:
