...
\begin{enumerate}
  ...
  \item Write your own function \ci{minimum(mylist)} that returns the smallest value of the items in a given list. Compare your result with the \Python{} function \ci{min()} described below (which can also calculate the minimum of two numbers).
...
\end{itemize}
  ...
  \item \textbf{Pie chart.} Write a function \ci{sector_graphics(mylist)} that displays the values of a list as a pie chart (a fixed size disk divided into sectors representing the values). 
...
\begin{enumerate}
  ...
  Write a function \ci{index_stock_exchange()}, without parameters, which returns a list of $365$ index values using this method.
