...
The command \ci{break} immediately leaves a loop \og{}while\fg{}
or a loop \og{}for\fg{}.
...
\begin{itemize}
  \item an \defi{input}, which groups variables that serve as \defi{parameters},
...
\begin{itemize}
  ...
  \item As soon as the program meets the instruction \ci{return}\index{return@\ci{return}}\index{function!return@\ci{return}}, the function stops and returns the result. There may be several times the instruction \ci{return} in a function but only one will be executed. It is also possible not to put an instruction \ci{return} if the function returns nothing.
  
  \item In the instructions of a function, you can of course use other functions.
   
% \item It is important to comment carefully on your programs. To document a function, you can describe what it does by starting with a \emph{docstring}\index{docstring@\emph{docstring}}}\index{function!docstring@\emph{docstring}, i.e. a description (in French) surrounded by three quotation marks: \ci{""" Ma fonction fait ceci et cela. """} to be placed just after the header.
\end{itemize}
...
\begin{itemize}
  \item The command \ci{draw(100)} traces my line, and as I only specified the length, the arguments \ci{width} and \ci{color} take the default values ($5$ and blue).
  
   \item The command \ci{draw(100, width=10)} traces my line with a new thickness (the color is the default one).
   
    \item The command \ci{draw(100, color="red")} traces my line with a new color (the thickness is the default one).  
    
     \item The command \ci{draw(100, width=10, color="red")} traces my line with a new thickness and a new color.
...
\begin{itemize}
  \item \ci{from math import *} \quad Imports all functions from the module \ci{math}. You are now able to use the sine function for example, by \ci{sin(0)}. This is the simplest method and it is the one we use in this book.
  
  \item \ci{import math} \quad Allows you to use the functions of the module \ci{math}. You can then access the sine function with \ci{math.sin(0)}. This is the officially recommended method to avoid conflicts between modules.
\end{itemize}
...
\begin{itemize}
  ...
  \item \ci{random} \quad simulates the random draw.
  ...
  \item \ci{tkinter} \quad allows you to display graphic windows.
  \item \ci{time} \quad for date, time and length of time.
...
\begin{itemize}
  ...
  \item \ci{x = sqrt(2)} \quad \Python{} returns the message \emph{NameError: name 'sqrt' is not defined}, you must import the module \ci{math} to be able to use the function \ci{sqrt()}.
