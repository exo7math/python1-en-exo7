...
The use of other mathematical functions requires the module \ci{math} which is called by the command:
...
\begin{itemize}
  \item \textbf{Chain.} \ci{str(number)}\quad converts a number (integer or floating point number) into a string.
...
\begin{itemize}
  \item \ci{string[i:j]}\quad returns the substring of characters with a rank of $i$ to $j-1$ from \ci{string}.
  
   Example: with \ci{string = "This is a string"}, \ci{string[2:7]} returns \ci{"is is"}.
   
  \item \ci{string[i:]}\quad returns characters with a rank of $i$ until the end of \ci{string}. 
  
  Example :
  \ci{string[5:]} returns \ci{"is a string"}.
  
  \item\ci{string[:j]}\quad returns characters from the beginning to the rank $j-1$ of \ci{string}. Example: \ci{string[:4]} returns \ci{"This"}.
...
\begin{itemize}
 ...
 \end{center}
  ...
  \begin{itemize}
    \item \ci{'\{:10\}'.format('Test')} \quad left aligned (on 10 characters)
...
\begin{itemize}
  \item \ci{string.upper()} returns a string in upper case.
...
Examples:
\begin{itemize}
    \item \ci{mylist1 = [5,4,3,2,1]} \quad a list of integers $5$.
    \item \ci{mylist2 = ["Friday","Saturday","Sunday"]} \quad a list of $3$ strings.
...
\begin{itemize}
    \item \ci{mylist[i]} \quad returns the element of rank $i$. Be careful, the rank starts at $0$.
...
\textbf{Slicing lists}
  ...
  \begin{itemize}
    \item \ci{mylist[a:b]} \quad returns the sublist of elements from the rank $a$ to the rank $b-1$.
    
    \item \ci{mylist[a:]} \quad returns the list of elements of rank $a$ until the end.
      
    \item \ci{mylist[:b]} \quad returns the list of items from the beginning to the rank $b-1$.
...
\medskip
  ...
  \begin{itemize}
    ...
    \item \ci{mylist[4:]} \quad returns \ci{["E","F","G"]}.  It's the same thing 
     than \ci{mylist[4:n]} where \ci{n = len(mylist)}.
  \end{itemize} 
...
\begin{itemize}
 ...
 Example: \ci{print("Line 1 \\n Line 2")} displays on two lines.
  ...
  \ci{Bob; 17; 13; 16}.
   ...
   \item \textbf{End of line.} \ci{print(...,end="...")} \quad changes the character placed at the end (by default it is the line break \ci{\\n}).
...
\ci{input()} \quad pauses the program and waits for the user to send a message on the keyboard (end by pressing the \og{}Enter\fg{} key). The message is a string.
...
\begin{itemize}
  \item \ci{fi = open("my_file.txt","r")} \quad opening in reading (\ci{"r"} = \emph{read}).
  \item \ci{fi = open("my_file.txt","w")} \quad opening in writing (\ci{"w"} = \emph{write}). The file is created if it does not exist, if it existed the previous content is first deleted.
  \item \ci{fi = open("my_file.txt","a")} \quad opening for writing, the data will be written at the end of the current data (\ci{"a"} = \emph{append}).
...
\begin{itemize}
  ...
  \item \ci{setheading(direction)} points in one direction ($0$ = right, $90$ = top, $-90$ = bottom, $180$ = left)
...
\begin{itemize}
  \item \ci{plt.plot(mylist)} traces the points of a list (in the form $(i,\ell_i)$)  that are linked by segments. 
  \item \ci{plt.plot(list_x,list_y)} traces the points of a list (in the form of $(x_i,y_i)$ where $x_i$ browses the first list and $y_i$ the second).    
  \item \ci{plt.scatter(x,y,color='red',s=100)} displays a point in $(x,y)$ (of a size \ci{s}).
...
\begin{itemize}
  ...
  \item The option \ci{font} allows you to define the font (i.e. the style and size of the characters). Here are some examples of fonts, it's up to you to test them:
  \begin{itemize}
...
\begin{itemize}
  \item The creation of the window is usual. The program ends with the launch by the command  \ci{mainloop()}.
...
Each time the left mouse button is clicked, \Python{} executes the function \ci{action_mouse_click}. (Note that there are no brackets for the call to the function.)
   ...
   \item Second key point: the \ci{action_mouse_click} function retrieves the click coordinates and then does two things here: it displays a small rectangle at the click location and displays the $(x,y)$ coordinates in the terminal window.
